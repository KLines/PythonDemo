...
**1、基本语法**
	...
	标识符：
		...
		* "双下划线"：
		  以双下划线开头的 ；以双下划线开头和结尾的 __foo__ 代表 Python 里特殊方法专用的标识，如 __init__() 代表类的构造函数
...
**11、面向对象**
	...
	类属性和实例属性：	
		...
		* 根据类创建的实例可以任意绑定属性和方法，如果绑定的方法中存在属性，绑定方法并且执行之后才能访问其属性
				1、绑定类属性和类方法	 
					# 给class绑定类方法和类属性
		...
		* __slots__变量使用：
				1、限制类的实例能添加的属性或者方法
				2、定义的属性仅对当前类实例起作用，对继承的子类是不起作用的
				3、类本身的属性的添加不受__slots__的限制
				4、给类的实例绑定方法时，不能添加__slots__限制之外的属性
				__slots__ = ('name', 'age'，'set_age') # 用tuple定义允许绑定的属性名称和方法名称
