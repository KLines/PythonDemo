...
* 类方法: 默认有个 cls 参数，可以被类和对象调用，需要加上 @classmethod 装饰器，不论任何情况,都是⽅法--method
* 普通方法: 默认有个self参数，代表的是类的实例且只能被对象调用，self 的名字并不是规定死的，也可以使用 this，但是最好还是按照约定是用 self。如果是实例访问就是⽅法，如果是类名访问就是函数
